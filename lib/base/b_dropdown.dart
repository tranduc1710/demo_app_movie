...
///
/// Used by [BDropdownButton.selectedItemBuilder].
typedef BDropdownButtonBuilder = List<Widget> Function(BuildContext context);
...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final BDropdownMenuItem<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item!;
...
class _DropdownMenuState<T> extends State<_DropdownMenu<T>> {
  ...
  Widget build(BuildContext context) {
    ...
    return FadeTransition(
      ...
      child: Container(
        constraints: BoxConstraints(maxHeight: sWidth),
        child: CustomPaint(
...
class _DropdownRoutePage<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // BDropdownButton.itemHeight is specified or BDropdownButton.itemHeight is null
...
class _MenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final BDropdownMenuItem<T>? item;
...
// The container widget for a menu item created by a [BDropdownButton]. It
// provides the default configuration for [BDropdownMenuItem]s, as well as a
// [BDropdownButton]'s hint and disabledHint widgets.
...
/// An item in a menu created by a [BDropdownButton].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class BDropdownMenuItem<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const BDropdownMenuItem({
    Key? key,
    this.onTap,
    this.value,
  ...
  ///
  /// Eventually returned in a call to [BDropdownButton.onChanged].
  final T? value;
}

/// An inherited widget that causes any descendant [BDropdownButton]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [BDropdownButton] widgets placed within material data tables, as
/// required by the material design specification.
class BDropdownButtonHideUnderline extends InheritedWidget {
  /// Creates a [BDropdownButtonHideUnderline]. A non-null [child] must
  /// be given.
  const BDropdownButtonHideUnderline({
    Key? key,
    required Widget child,
  })  : assert(child != null),
        super(key: key, child: child);

  /// Returns whether the underline of [BDropdownButton] widgets should
  /// be hidden.
  static bool at(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<BDropdownButtonHideUnderline>() != null;
  }

  @override
  bool updateShouldNotify(BDropdownButtonHideUnderline oldWidget) => false;
}
...
/// Typically, an enum is used. Each [BDropdownMenuItem] in [items] must be
...
///
/// This sample shows a `BDropdownButton` with a large arrow icon,
...
///       .map<BDropdownMenuItem<String>>((String value) {
///         return BDropdownMenuItem<String>(
///           value: value,
///           child: Text(value),
///         );
///       })
...
/// See also:
///
///  * [BDropdownMenuItem], the class used to represent the [items].
///  * [BDropdownButtonHideUnderline], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [ElevatedButton], [TextButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class BDropdownButton<T> extends StatefulWidget {
  /// Creates a dropdown button.
  ///
  /// The [items] must have distinct values. If [value] isn't null then it
  /// must be equal to one of the [BDropdownMenuItem] values. If [items] or
  ...
  BDropdownButton({
    ...
    // BDropdownButtonFormField.
  })  : assert(
          items == null ||
              items.isEmpty ||
              value == null ||
              items.where((BDropdownMenuItem<T> item) {
                    return item.value == value;
                  }).length ==
                  1,
          "There should be exactly one item with [BDropdownButton]'s value: "
          '$value. \n'
          'Either zero or 2 or more [BDropdownMenuItem]s were detected '
          'with the same value',
        ),
  ...
  final List<BDropdownMenuItem<T>>? items;

  /// The value of the currently selected [BDropdownMenuItem].
  ///
  /// If [value] is null and the button is enabled, [hint] will be displayed
  /// if it is non-null.
  ///
  ...
  ///
  /// If the [onChanged] callback is null or the list of [BDropdownButton.items]
  /// is null then the dropdown button will be disabled, i.e. its arrow will be
  /// displayed in grey and it will not respond to input. A disabled button
  /// will display the [BDropdownButton.disabledHint] widget if it is non-null.
  /// If [BDropdownButton.disabledHint] is also null but [BDropdownButton.hint] is
  /// non-null, [BDropdownButton.hint] will instead be displayed.
  /// {@endtemplate}
  ...
  /// [BDropdownMenuItem]s in [items].
  ///
  /// When a [BDropdownMenuItem] is selected, the widget that will be displayed
  /// from the list corresponds to the [BDropdownMenuItem] of the same index
  /// in [items].
  ///
  /// {@tool dartpad --template=stateful_widget_scaffold}
  ///
  /// This sample shows a `BDropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [BDropdownMenuItem].
  ///
  /// ```dart
  ...
  /// {@end-tool}
  ///
  /// If this callback is null, the [BDropdownMenuItem] from [items]
  /// that matches [value] will be displayed.
  final BDropdownButtonBuilder? selectedItemBuilder;
  ...
  /// {@tool dartpad --template=stateful_widget_scaffold}
  ///
  /// This sample shows a `BDropdownButton` with a dropdown button text style
  ...
  ///         }).toList();
  ///       },
  ///       items: options.map<BDropdownMenuItem<String>>((String value) {
  ///         return BDropdownMenuItem<String>(
  ///           value: value,
  ///           child: Text(value),
  ///         );
...
class _DropdownButtonState<T> extends State<BDropdownButton<T>> with WidgetsBindingObserver {
  int? _selectedIndex;
  ...
  @override
  void didUpdateWidget(BDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    if (widget.items == null || widget.items!.isEmpty || (widget.value == null && widget.items!.where((BDropdownMenuItem<T> item) => item.value == widget.value).isEmpty)) {
      _selectedIndex = null;
      return;
    }

    assert(widget.items!.where((BDropdownMenuItem<T> item) => item.value == widget.value).length == 1);
  ...
  Widget build(BuildContext context) {
    ...
    if (!BDropdownButtonHideUnderline.at(context)) {
...
/// A convenience widget that makes a [BDropdownButton] into a [FormField].
class BDropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [BDropdownButton] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  BDropdownButtonFormField({
    Key? key,
    required List<BDropdownMenuItem<T>>? items,
    BDropdownButtonBuilder? selectedItemBuilder,
    T? value,
    Widget? hint,
    Widget? disabledHint,
    this.onChanged,
    ...
    EdgeInsets? margin,
          ...
          items == null ||
              items.isEmpty ||
              value == null ||
              items.any((BDropdownMenuItem<T> item) {
                return item.value == value;
              }),
          "There should be exactly one item with [BDropdownButton]'s value: "
          '$value. \n'
              ...
              child: Builder(builder: (BuildContext context) {
                return Container(
                  decoration: BoxDecoration(
                    color: colorBackground ?? themeData.backgroundColor,
                    border: border ?? Border.all(color: Colors.black.withOpacity(.3)),
                    borderRadius: BorderRadius.circular(radius ?? Constant.brButtonAndTF),
                    boxShadow: listShadow ?? [],
                  ),
                  margin: margin,
                  padding: padding ?? EdgeInsets.symmetric(horizontal: sWValue(.03)),
                  child: InputDecorator(
                    ...
                    child: BDropdownButtonHideUnderline(
                      child: BDropdownButton<T>(
                        items: items,
                        selectedItemBuilder: selectedItemBuilder,
                        value: state.value,
                        hint: hint ?? AutoSizeText("Chọn giá trị"),
...
class _DropdownButtonFormFieldState<T> extends FormFieldState<T> {
  @override
  BDropdownButtonFormField<T> get widget => super.widget as BDropdownButtonFormField<T>;
  ...
  @override
  void didUpdateWidget(BDropdownButtonFormField<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
