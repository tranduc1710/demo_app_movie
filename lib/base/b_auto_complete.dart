...
class AutoCompleteTextFieldState<T> extends State<AutoCompleteTextField> {
  ...
  int minLength;
  bool submitOnSuggestionTap, clearOnSubmit = false;
  ...
  void updateDecoration(InputDecoration? decoration, List<TextInputFormatter>? inputFormatters, TextCapitalization? textCapitalization, TextStyle? style,
  ...
  void triggerSubmitted({submittedText}) {
    // if (textSubmitted != null) submittedText == null ? textSubmitted!(currentText) : textSubmitted!(submittedText);
                                  ...
                                  onTap: () {
                                    setState(() {
                                      if (submitOnSuggestionTap) {
                                        String newText = getText(suggestion).toString();
                                        textField!.controller!.text = newText;
                                        textField!.focusNode!.unfocus();
                                        if (itemSubmitted != null) itemSubmitted!(suggestion);
                                        if (clearOnSubmit) {
                                          clear();
                                        }
                                      } else {
  ...
  List<T> getSuggestions(List<T> suggestions, Comparator<T>? sorter, Filter<T>? filter, int maxAmount, String? query) {
    ...
    if (query.trim().isNotEmpty) suggestions = suggestions.where((item) => filter!(item, query)).toList();
...
class SimpleAutoCompleteTextField<O> extends AutoCompleteTextField<O> {
  final StringCallback? textChanged;

  // textSubmitted;
  final Function(O)? textSubmitted;
  final String Function(O) getText;
  final Widget Function(O)? buildItem;
  final int minLength;
  final ValueSetter<bool>? onFocusChanged;
  final TextEditingController? controller;
  final FocusNode? focusNode;
  ...
  SimpleAutoCompleteTextField(
      ...
      required GlobalKey<AutoCompleteTextFieldState<O>> key,
      required List<O> suggestions,
      int suggestionsAmount: 5,
      bool submitOnSuggestionTap: false,
  ...
  @override
  State<StatefulWidget> createState() => new AutoCompleteTextFieldState<O>(
        suggestions,
        textChanged,
        getText,
        // textSubmitted,
        ...
        (context, item) {
          if (buildItem != null)
            ...
            return new Padding(padding: EdgeInsets.all(sWValue(.025)), child: new Text(getText(item)));
        },
        (a, b) {
          return getText(a).compareTo(getText(b));
        },
        (item, query) {
          return getText(item).toLowerCase().startsWith(query.toLowerCase());
        },
        suggestionsAmount,
        submitOnSuggestionTap,
        clearOnSubmit,
